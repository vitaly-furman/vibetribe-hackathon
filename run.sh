    ...
    # macOS adjustments: Use echo for decorative purposes
    echo "
    ...
    # macOS adjustments: Replace 'truncate' with '>:' for file truncation
    :> ./gerobug_dashboard/gerobug_host

    # macOS adjustments: Replace 'ip' command with alternative for macOS
    # This example uses 'ipconfig getifaddr en0'; adjust network interface as necessary
    IP=$(ipconfig getifaddr en0)
    echo "Server Public IP : $IP"

    # The user interaction part remains the same

    # macOS adjustments: For file editing, ensure compatibility or instruct macOS users on alternatives

    # Package management and Docker installation instructions should be modified for macOS users

    # The rest of the script can largely remain unchanged, focusing on adjusting command utilities and paths as necessary for macOS environments
